...
$tracking_page_name="livingroom_edit.php";
...
$content = 'Edit livingroom_device';       //variable for edit user, named £content
if (isset($_POST['submit'])) {      //update database grupp17_users, collect data from database, fname,lname and email, where id, collect id from database
 $query = <<<END
UPDATE livingroom_devices
SET livingroom_name = '{$_POST['livingroom_name']}'
WHERE id = '{$_GET['id']}'
END;
 ...
 $query = <<<END
SELECT * FROM livingroom_devices
 WHERE id = '{$_GET['id']}'
END;
 ...
 $content = <<<END
<form method="post" action="livingroom_edit.php?id={$row->id}" style="padding-left:40%;">
<p> Please enter your new device name! </p>
 <input type="text" name="livingroom_name" required placeholder ="Device name"><br>

 <input type="submit" name="submit" value="save" onclick="alert('Device added!')">
 </form>
